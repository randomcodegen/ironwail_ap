...
The console sends lines starting with `!` to the AP server (for example, `!help`, `!hint Rocketlauncher`, or `!hint_location e1m1 Green Armor (1)`). Press Tab after `!hint ` to complete item names, or after `!hint_location ` to complete location names. The commands `hint` and `hint_location` also work without `!`. Hint commands and item/location matching are case-insensitive. Names with spaces do not need quotes; repeated Tab / Shift+Tab cycles matches.
